...
github.com). GLFW на Linux по умолчанию собирается с двумя бэкендами — X11 и
Wayland, и без `wayland-scanner` падает на
`Failed to find wayland-scanner`. Поэтому Wayland включается только тогда,
когда его инструменты и клиентские библиотеки реально стоят, иначе игра
собирается с X11 — ставить что-то дополнительно не нужно.

Проверку можно перебить вручную: `-DGAME3D_GLFW_WAYLAND=ON` (требовать Wayland,
пакеты: `wayland-devel wayland-protocols libxkbcommon-devel` на Fedora или
`libwayland-dev wayland-protocols libxkbcommon-dev` на Debian/Ubuntu) или
`-DGAME3D_GLFW_WAYLAND=OFF` (только X11).
...
Игра открывается главным меню: **НАЧАТЬ** и **ВЫЙТИ** (внизу).

**НАЧАТЬ** показывает список карт — файлов `.map` и `.txt`, которые лежат рядом
с исполняемым файлом игры (там же, где `game3d` или `game3d.exe`). Клик по
строке запускает карту, первая строка **БЕЗ КАРТЫ** запускает процедурный мир.
Если файлов много, список крутится колесом мыши. Кнопка **X** в углу панели
закрывает выбор карты и возвращает в главное меню.

**Esc** в игре больше не закрывает её: открывается пауза с кнопкой
**ЗАКРЫТЬ МЕНЮ** сверху и **ВЫЙТИ** внизу. **ВЫЙТИ** в паузе заканчивает партию
и возвращает в главное меню (оттуда можно выбрать другую карту), а **ВЫЙТИ** в
главном меню закрывает игру. Esc в меню работает как «назад», в главном меню —
как выход. Игра закрывается и крестиком окна.

Если карту передать в командной строке (`game3d текстура.raw карта.map`), игра
начинается сразу, минуя меню.

Карта ищется в каталоге исполняемого файла: на Linux это `/proc/self/exe`, на
Windows — `GetModuleFileNameA`. Если определить каталог не удалось, берётся
текущий.
